...
def build_plan(company_code: str, site: str, employee_id: int, d_from: date, d_to: date) -> dict:
    ...
    for sh in affected:
        d = sh.date
        pk = sh.post_key
        opts = {"calls": [], "moves": [], "reinforcements": []}
        for e in all_emps:
            ...
            if wh + sh.hours > R["week_max_hours"]:
                continue
            if wh + sh.hours > e.weekly_hours + R.get("overtime_tolerance", 2):
                base -= 10
            if not working:
                if d.weekday() in service.e_list(e, "cfa_days"):
                    continue
                if not _rest_ok(company_code, e.id, d, sh.start, R):
                    continue
                item = {"employee_id": e.id, "name": service.full_name(e), "site": e.site, "phone": e.phone, "telegram": e.telegram,
                        ...
                        "score": base + (4 if d.weekday() in service.e_list(e, "days_off") else 0), "status": "à contacter"}
                if e.site == site:
...
def current_step(plan: dict, shift_entry: dict) -> dict:
    ...
    if tranche:
        ...
        reinf = shift_entry["options"]["reinforcements"][:n]
        if reinf and idx <= len(calls_all) // n + 1:
...
def record_answer(inc: PlIncident, shift_id: int, employee_id: int, outcome: str, by: str = None) -> PlIncident:
    ...
    if outcome == "next":
        ...
        for grp in ("calls", "reinforcements", "moves"):
...
def _apply_assign(inc: PlIncident, entry: dict, cand: dict, grp: str) -> None:
    ...
    with Session(engine) as s:
        ...
        if grp == "moves" and cand.get("shift_id"):
