...
class OwItem(SQLModel, table=True):
    ...
    abv_status: Optional[str] = None                     # estime (déduit / recherche publique) ou confirme (étiquette, Alix)
    abv_source: Optional[str] = None                     # d'où vient le degré (fiche technique, Alix le …, étiquette)
    customs_confirmed_at: Optional[datetime] = None      # données douanières (couleur, degré, millésime, code SH) confirmées vin par vin
    customs_confirmed_by: Optional[str] = None
