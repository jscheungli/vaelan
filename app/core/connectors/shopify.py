...
Clés : SHOPIFY_<CODE>_SHOP (ex. « owine » ou « owine.myshopify.com »), SHOPIFY_<CODE>_TOKEN (jeton d'accès Admin API
« shpat_… » d'une custom app), SHOPIFY_<CODE>_APIVERSION (optionnel, défaut ci-dessous) — variables Render,
ou section "shopify" de ~/.config/vaelan/credentials.json en local (chargée par localenv).
...
class ShopifyClient:
    def __init__(self, shop: str, token: str, api_version: str = DEFAULT_API_VERSION):
        ...
        self._h = {"X-Shopify-Access-Token": token, "Content-Type": "application/json", "Accept": "application/json"}
    ...
    def gql(self, query: str, variables: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for attempt in range(6):
            with httpx.Client(timeout=60) as c:
                r = c.post(self.url, headers=self._h, json={"query": query, "variables": variables or {}})
...
def for_company(code: str) -> Optional[ShopifyClient]:
    ...
    if not (shop and token):
        return None
    return ShopifyClient(shop, token, os.getenv(f"SHOPIFY_{k}_APIVERSION") or DEFAULT_API_VERSION)
